...
async def _should_migrate(engine: AsyncEngine) -> bool:
    """
    检查 content_version.version_num 是否为 2
    """
    new_content_version = 3
    metadata = MetaData()
    version_table = Table("content_version", metadata, Column("version_num", Integer))
    async with AsyncSession(engine) as session:
        result = await session.execute(select(version_table.c.version_num).limit(1))
        row = result.fetchone()
        return row is not None and row[0] == new_content_version

async def upgrade_content_db_2_to_3() -> bool:
    db_path = get_plugin_data_dir() / "content.db"
    if not db_path.exists():
    ...
    try:
        if not await _should_migrate(engine):
            return True
